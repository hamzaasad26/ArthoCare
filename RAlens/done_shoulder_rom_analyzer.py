"""
Shoulder Joint ROM Analysis — Rheumatoid Arthritis Detection
MediaPipe Pose Tracking | AAOS Clinical Standards

Flow (per shoulder):
  Neutral Baseline → Flexion → Extension → Abduction → report screen (12s)
  LEFT shoulder first, then RIGHT shoulder automatically.
...
Author: Shoulder ROM Analysis System | 2025
"""
...
class TestState(Enum):
    NEUTRAL_BASELINE = 0
    FLEXION_TEST     = 1   # raise arm forward (0° → 180°)
    EXTENSION_TEST   = 2   # push arm backward (0° → 60°)
    ABDUCTION_TEST   = 3   # raise arm sideways (0° → 180°)
    COMPLETE         = 4

# AAOS thresholds
THRESH = {
    'neutral_max':       20,   # arm at rest should be < 20°
    'flexion_normal':   160,   # AAOS normal flexion ≥ 160° (full = 180°)
    'flexion_min':      120,   # acceptable minimum
    'extension_normal':  50,   # AAOS normal extension ≥ 50° (full = 60°)
    'extension_min':     30,   # acceptable minimum
    'abduction_normal': 160,   # AAOS normal abduction ≥ 160° (full = 180°)
    'abduction_min':    120,   # acceptable minimum
}
...
# BGR colours (same palette as elbow analyzer)
C_CYAN   = (255, 220,   0)
C_GREEN  = (  0, 230,   0)
C_ORANGE = (  0, 160, 255)
C_RED    = (  0,   0, 220)
...
class ShoulderResult:
    ...
    abduction_normal:  bool
    flexion_deficit:   float
    extension_deficit: float
    abduction_deficit: float
    flare_risk:        float
    risk_level:        str
    timestamp:         str
...
# ═══════════════════════════════════════════════════════════
#  GEOMETRY  — shoulder angle relative to body vertical
# ═══════════════════════════════════════════════════════════
...
def shoulder_abduction_angle(shoulder: np.ndarray, elbow: np.ndarray,
                              hip: np.ndarray) -> float:
    """
    Lateral (frontal-plane) elevation from body.
    0° = arm hanging at side, 180° = overhead.
    Uses the torso vertical (hip→shoulder) as reference.
    """
    torso   = shoulder - hip                       # points upward
    arm_vec = elbow    - shoulder                  # points toward elbow
    # project both onto frontal plane (zero out depth / z)
    t2d = np.array([torso[0],   torso[1],   0.0])
    a2d = np.array([arm_vec[0], arm_vec[1], 0.0])
    return max(0.0, _angle_between(t2d, a2d))


def shoulder_flexion_angle(shoulder: np.ndarray, elbow: np.ndarray,
                            hip: np.ndarray) -> float:
    """
    Sagittal-plane elevation (forward flexion OR backward extension).
    Returns the angle of the arm relative to the body in the side view.
    We use the x-z plane (depth + horizontal).
    """
    torso   = shoulder - hip
    arm_vec = elbow    - shoulder
    # sagittal plane: y + z (ignore lateral x component)
    t2d = np.array([0.0, torso[1],   torso[2]])
    a2d = np.array([0.0, arm_vec[1], arm_vec[2]])
    return max(0.0, _angle_between(t2d, a2d))


def get_arm_landmarks(pose_lm, side: str):
    """Return (shoulder, elbow, wrist, hip, visibility) as np arrays."""
    P = mp.solutions.pose.PoseLandmark
    if side == 'left':
        si, ei, wi, hi = (P.LEFT_SHOULDER.value,  P.LEFT_ELBOW.value,
                          P.LEFT_WRIST.value,      P.LEFT_HIP.value)
    else:
        si, ei, wi, hi = (P.RIGHT_SHOULDER.value, P.RIGHT_ELBOW.value,
                          P.RIGHT_WRIST.value,     P.RIGHT_HIP.value)
    lm  = pose_lm.landmark
    ...
    vis = min(lm[si].visibility, lm[ei].visibility,
              lm[wi].visibility,  lm[hi].visibility)
    return sh, el, wr, hp, vis


def compute_angle(pose_lm, side: str, state: TestState) -> float:
    """
    Returns the relevant angle for the current test state.
    Neutral / Abduction use lateral (frontal) angle.
    Flexion / Extension use sagittal angle.
    """
    sh, el, wr, hp, _ = get_arm_landmarks(pose_lm, side)
    if state in (TestState.NEUTRAL_BASELINE, TestState.ABDUCTION_TEST):
        return shoulder_abduction_angle(sh, el, hp)
    else:  # FLEXION or EXTENSION
        return shoulder_flexion_angle(sh, el, hp)


# ═══════════════════════════════════════════════════════════
#  POSTURE VALIDATION  — per state
# ═══════════════════════════════════════════════════════════
...
def validate(angle: float, state: TestState) -> Tuple[bool, str, str]:
    ...
    if state == TestState.NEUTRAL_BASELINE:
        if angle <= THRESH['neutral_max']:
            return True, f"Good – arm at side  ({angle:.0f} deg)", ""
        diff = angle - THRESH['neutral_max']
        return False, f"Lower arm to side  ({angle:.0f} deg  need < {THRESH['neutral_max']} deg)", \
               f"  Let arm hang naturally – lower by ~{diff:.0f} deg"

    elif state == TestState.FLEXION_TEST:
        if angle >= 110:
            return True,  f"Good flexion  ({angle:.0f} deg)", ""
        elif angle >= 60:
            return True,  f"Keep raising arm forward  ({angle:.0f} deg)", \
                   f"  Lift arm straight in front and continue up"
        else:
            return False, f"Raise arm forward  ({angle:.0f} deg  need > 60 deg)", \
                   f"  Lift your arm forward in front of your body"

    elif state == TestState.EXTENSION_TEST:
        if angle >= 25:
            return True,  f"Good extension  ({angle:.0f} deg)", ""
        elif angle >= 10:
            return True,  f"Keep pushing arm back  ({angle:.0f} deg)", \
                   f"  Push your arm further behind your body"
        else:
            return False, f"Push arm backward  ({angle:.0f} deg  need > 10 deg)", \
                   f"  Swing your arm behind your back"

    elif state == TestState.ABDUCTION_TEST:
        if angle >= 110:
            return True,  f"Good abduction  ({angle:.0f} deg)", ""
        elif angle >= 60:
            return True,  f"Keep raising arm sideways  ({angle:.0f} deg)", \
                   f"  Continue lifting sideways toward overhead"
        else:
            return False, f"Raise arm sideways  ({angle:.0f} deg  need > 60 deg)", \
                   f"  Lift your arm out to the side (like a T-pose)"

    return True, "Position OK", ""


# ═══════════════════════════════════════════════════════════
#  GUI HELPERS  (identical style to elbow_rom_analyzer)
# ═══════════════════════════════════════════════════════════
...
def arc_gauge(img, cx, cy, rad, angle, max_a=180):
    ...
    if angle > 2:
        ...
        cv2.ellipse(img, (cx,cy), (rad,rad), -90, 0, int(min(angle, max_a)),
                    col, 8, cv2.LINE_AA)
...
def draw_skeleton(frame, pose_lm, side, w, h, angle):
    ...
    cv2.line(frame, hp, sp, (100, 0, 0),    8, cv2.LINE_AA)   # torso glow
    cv2.line(frame, hp, sp, C_PURPLE,        3, cv2.LINE_AA)   # torso
    ...
    for pt, col, r in [(sp, C_YELLOW, 11), (ep, C_CYAN, 10),
                        (wp, C_LBLUE, 9),  (hp, C_PURPLE, 8)]:
        cv2.circle(frame, pt, r+3, (0,0,0), -1, cv2.LINE_AA)
        cv2.circle(frame, pt, r,   col,     -1, cv2.LINE_AA)
    txt(frame, f"{angle:.0f} deg", sp[0]+14, sp[1]-18, 0.75, C_YELLOW, 2, bold=True)
...
def draw_live(frame, pose_lm, angle, state, side,
              ok, msg, hint, max_flex, max_ext, max_abd,
    ...
    step_map = {
        TestState.NEUTRAL_BASELINE: ("STEP 1/4", "Hold arm relaxed at your side – neutral position"),
        TestState.FLEXION_TEST:     ("STEP 2/4", "Raise arm FORWARD (flexion) as high as possible"),
        TestState.EXTENSION_TEST:   ("STEP 3/4", "Push arm BACKWARD (extension) behind your body"),
        TestState.ABDUCTION_TEST:   ("STEP 4/4", "Raise arm SIDEWAYS (abduction) as high as possible"),
    }
    ...
    txt(frame, f"{side.upper()} SHOULDER", w-122, 42, 0.52, C_WHITE, 2)

    # ARC GAUGE
    gcx, gcy = w-95, h-120
    arc_gauge(frame, gcx, gcy, 72, angle, max_a=180)
    txt(frame, f"{angle:.0f} deg", gcx-36, gcy+20, 0.82, C_YELLOW, 2, bold=True)
    txt(frame, "ANGLE",           gcx-28, gcy+42, 0.44, C_WHITE,  1)

    # POSTURE FEEDBACK
    fy = h-105
    rrect(frame, 8, fy, 560, h-8, 8,
    ...
    # PROGRESS BARS
    rrect(frame, 8, 82, 230, 248, 8, (18,18,32), alpha=0.78)
    txt(frame, "PROGRESS", 18, 106, 0.48, C_CYAN, 1)
    bw = 195

    def _bar(label, val, ref, yy, good_fn):
        txt(frame, label, 18, yy, 0.44, C_WHITE, 1)
        fp_ = min(val / ref, 1.0)
    ...
    _bar(f"Flexion:   {max_flex:.0f} deg",
         max_flex, THRESH['flexion_normal'],    122,
         lambda v: v >= THRESH['flexion_min'])
    _bar(f"Extension: {max_ext:.0f} deg",
         max_ext,  THRESH['extension_normal'],  162,
         lambda v: v >= THRESH['extension_min'])
    _bar(f"Abduction: {max_abd:.0f} deg",
         max_abd,  THRESH['abduction_normal'],  202,
         lambda v: v >= THRESH['abduction_min'])
...
def draw_report(result: ShoulderResult, secs_left: float):
    img = np.zeros((WINDOW_H, WINDOW_W, 3), dtype=np.uint8)
    img[:] = (16, 16, 26)

    # Header
    rrect(img, 0, 0, WINDOW_W, 56, 0, (28,28,46), alpha=1.0)
    txt(img, "SHOULDER ROM REPORT", WINDOW_W//2-160, 38,
    ...
    rrect(img, 28, 68, 220, 106, 6, sc, alpha=0.92)
    txt(img, f"{result.side.upper()} SHOULDER", 40, 96, 0.64, C_WHITE, 2)
    ...
    def mrow(label, val, ref, ok_, yy):
        ...
        txt(img, ref,             380,           yy,  0.42, (155,155,155),  1)
        txt(img, tag, WINDOW_W-128, yy,          0.50, tc,                  2)

    mrow("Max Flexion   (arm forward)",
         result.max_flexion,
         f">= {THRESH['flexion_normal']} deg",
         result.flexion_normal, y);  y += 36

    mrow("Max Extension (arm backward)",
         result.max_extension,
         f">= {THRESH['extension_normal']} deg",
         result.extension_normal, y); y += 36

    mrow("Max Abduction (arm sideways)",
         result.max_abduction,
         f">= {THRESH['abduction_normal']} deg",
         result.abduction_normal, y); y += 44
    ...
    overall_ok = result.flexion_normal and result.extension_normal and result.abduction_normal

    if overall_ok:
        bg2  = (18,44,18); lc = C_GREEN
        lines = [
            "  Shoulder ROM within AAOS normal limits.",
            f"  Flexion {result.max_flexion:.0f} deg  |  Extension {result.max_extension:.0f} deg  |  Abduction {result.max_abduction:.0f} deg"
        ]
    else:
        ...
        if not result.extension_normal:
            lines.append(
                f"  Extension restricted – {result.max_extension:.0f} deg "
                f"(deficit {result.extension_deficit:.0f} deg below {THRESH['extension_normal']} deg)"
            )
        if not result.abduction_normal:
            lines.append(
                f"  Abduction restricted – {result.max_abduction:.0f} deg "
                f"(deficit {result.abduction_deficit:.0f} deg below {THRESH['abduction_normal']} deg)"
            )

    rrect(img, 32, y-4, WINDOW_W-32, y+len(lines)*30+10, 6, bg2, alpha=0.88)
    for ln in lines:
        txt(img, ln, 42, y+20, 0.48, lc, 1); y += 30
    y += 20
...
# ═══════════════════════════════════════════════════════════
#  CLINICAL ANALYSIS  — risk score identical in style to elbow
# ═══════════════════════════════════════════════════════════

def analyze(neutral, max_flex, max_ext, max_abd, side) -> ShoulderResult:
    neutral   = float(neutral)
    max_flex  = float(max_flex)
    max_ext   = float(max_ext)
    max_abd   = float(max_abd)

    flex_ok = max_flex >= THRESH['flexion_min']
    ext_ok  = max_ext  >= THRESH['extension_min']
    abd_ok  = max_abd  >= THRESH['abduction_min']

    fd  = max(0.0, THRESH['flexion_normal']   - max_flex)
    ed  = max(0.0, THRESH['extension_normal'] - max_ext)
    abd_d = max(0.0, THRESH['abduction_normal'] - max_abd)

    # ── Heuristic flare-up risk (0–100 %) ──
    # Neutral excess contribution
    neutral_excess = max(0.0, neutral - THRESH['neutral_max'])
    nsev = min(neutral_excess / 20.0, 1.0)

    # Movement-deficit contributions (only counted when actually restricted)
    fsev  = min(fd    / 40.0, 1.0) if not flex_ok else 0.0
    esev  = min(ed    / 20.0, 1.0) if not ext_ok  else 0.0
    asev  = min(abd_d / 40.0, 1.0) if not abd_ok  else 0.0

    # Weights: abduction 40 %, flexion 30 %, extension 20 %, neutral 10 %
    sev_score = 0.10*nsev + 0.30*fsev + 0.20*esev + 0.40*asev
    ...
    if not ext_ok:
        abn.append(
            f"Restricted extension: {max_ext:.0f} deg "
            f"(normal >= {THRESH['extension_normal']} deg, deficit {ed:.0f} deg)"
        )
    if not abd_ok:
        abn.append(
            f"Reduced abduction: {max_abd:.0f} deg "
            f"(normal >= {THRESH['abduction_normal']} deg, deficit {abd_d:.0f} deg)"
        )
    ...
    if flare_risk < 25:
        ...
        suggestions = [
            ...
            "Routine shoulder ROM check every 3–6 months.",
        ]
    elif flare_risk < 60:
        risk_level = "moderate"
        suggestions = [
            "Increase supervised shoulder ROM and strengthening exercises.",
            "Monitor for pain, swelling or morning stiffness in the shoulder.",
        ...
        suggestions = [
            ...
            "Monitor closely for flare symptoms (pain, swelling, warmth, limited reach).",
        ]
    ...
    return ShoulderResult(
        ...
        abduction_normal=bool(abd_ok),
        flexion_deficit=fd,
        extension_deficit=ed,
        abduction_deficit=abd_d,
...
def save_report(folder, pid, left_r, right_r):
    ...
    jp = os.path.join(folder, f"{pid}_shoulder_report.json")
    with open(jp, 'w') as f:
        json.dump(
            {
                'patient_id': pid,
                'test_date':  ts,
                'left_shoulder':  asdict(left_r)  if left_r  else None,
                'right_shoulder': asdict(right_r) if right_r else None,
                'reference': {
                    'flexion_normal_deg':   THRESH['flexion_normal'],
                    'extension_normal_deg': THRESH['extension_normal'],
                    'abduction_normal_deg': THRESH['abduction_normal'],
    ...
    tp = os.path.join(folder, f"{pid}_shoulder_summary.txt")
    with open(tp, 'w', encoding='utf-8') as f:
        sep = "=" * 60
        f.write(sep + "\n  SHOULDER ROM ANALYSIS REPORT\n" + sep + "\n")
        f.write(f"  Patient ID : {pid}\n  Test Date  : {ts}\n" + sep + "\n\n")
        for res, lbl in [(left_r, "LEFT SHOULDER"), (right_r, "RIGHT SHOULDER")]:
            if not res: continue
            f.write(f"  {lbl}\n  " + "─"*44 + "\n")
            ...
            f.write(
                f"  Max Abduction: {res.max_abduction:.1f} deg  (normal >= {THRESH['abduction_normal']} deg)"
                f"  -> {'NORMAL' if res.abduction_normal else f'REDUCED deficit {res.abduction_deficit:.0f} deg'}\n"
            )
            all_ok = res.flexion_normal and res.extension_normal and res.abduction_normal
            f.write(
                f"  Flare-up risk: {res.flare_risk:.0f}%  ({res.risk_level.upper()} risk, heuristic)\n"
            )
            f.write(
                f"  Status       : {'NORMAL' if all_ok else 'RESTRICTION DETECTED'}\n"
            )
        ...
        if not issues:
            f.write("  - Shoulder ROM normal. Continue treatment plan.\n")
            ...
            f.write("  - Shoulder stretching 3x daily, 10 reps each direction.\n")
        ...
        f.write(f"  Abduction : >= {THRESH['abduction_normal']} deg\n" + sep + "\n")
...
# ═══════════════════════════════════════════════════════════
#  SINGLE-SHOULDER ANALYZER  (mirrors ArmAnalyzer in elbow)
# ═══════════════════════════════════════════════════════════

class ShoulderAnalyzer:
    def __init__(self, cap, pose, side: str, pid: str, out: str):
        self.cap  = cap
        self.pose = pose
        self.side = side
        self.pid  = pid
    ...
    def _plateaued(self, a: float) -> bool:
        """Only enforced on movement tests (not neutral)."""
    ...
    def run(self) -> Optional[ShoulderResult]:
        while True:
            ...
            # ── COMPLETE → show report screen ──
            if self.state == TestState.COMPLETE:
                result   = analyze(self.neutral, self.max_flex,
                                   self.max_ext, self.max_abd, self.side)
                deadline = time.time() + REPORT_SECS
                while time.time() < deadline:
                    sl = deadline - time.time()
                    cv2.imshow("Shoulder ROM Analyzer", draw_report(result, sl))
            ...
            if res.pose_landmarks:
                sh, el, wr, hp, vis = get_arm_landmarks(res.pose_landmarks, self.side)
                if vis > 0.35:
                    a = compute_angle(res.pose_landmarks, self.side, self.state)

                    # accumulate maxima
                    if self.state == TestState.FLEXION_TEST:
                        self.max_flex = max(self.max_flex, a)
                    elif self.state == TestState.EXTENSION_TEST:
                        self.max_ext  = max(self.max_ext,  a)
                    elif self.state == TestState.ABDUCTION_TEST:
                        self.max_abd  = max(self.max_abd,  a)
                    ...
                    if ready:
                        ...
                        if rem <= 0:
                            ...
                            draw_skeleton(cap_frame, res.pose_landmarks,
                                          self.side, WINDOW_W, WINDOW_H, a)
                    ...
                    frame = draw_live(
                        ...
                        ok, msg, hint,
                        self.max_flex, self.max_ext, self.max_abd,
                        cd, stable, WINDOW_W, WINDOW_H
                    )
                else:
                    rrect(frame, 8, WINDOW_H-68, 510, WINDOW_H-8, 6,
                          (55,0,0), 0.8)
                    txt(frame, "Move closer / improve lighting",
                        18, WINDOW_H-30, 0.58, C_RED, 2)
            else:
                rrect(frame, 8, WINDOW_H-68, 580, WINDOW_H-8, 6,
                      (55,0,0), 0.8)
                txt(frame, "Body not detected — step back a little",
                    18, WINDOW_H-30, 0.58, C_RED, 2)

            cv2.imshow("Shoulder ROM Analyzer", frame)
...
def main():
    ...
    cam = select_camera()
    out = os.path.join("shoulder_output", pid)
    ...
    cv2.namedWindow("Shoulder ROM Analyzer", cv2.WINDOW_NORMAL)
    cv2.resizeWindow("Shoulder ROM Analyzer", WINDOW_W, WINDOW_H)

    print("  Starting LEFT shoulder test...")
    left_r = ShoulderAnalyzer(cap, pose, 'left',  pid+"_L", out).run()

    right_r = None
    if left_r is not None:
        print("  Left done — starting RIGHT shoulder test...")
        time.sleep(1.2)
        right_r = ShoulderAnalyzer(cap, pose, 'right', pid+"_R", out).run()

    if left_r or right_r:
        save_report(out, pid, left_r, right_r)
        print(f"\n  All files saved: shoulder_output/{pid}/")
